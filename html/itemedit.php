...
$sql = "SELECT itemId,itemInfo,quan,placeId,Image FROM Item where itemId ='$itemId'";
...
$placeres=mysqli_query($connection, $placesql);
if ($placeres) {
    // 使用 mysqli_num_rows 方法取得資料列數，並判斷是否大於0
    // 取得的量大於0代表有資料@
    if (mysqli_num_rows($placeres) > 0) {
        
        // while迴圈會根據查詢到的資料筆數，迭代到結束為止
        while ($placerow = mysqli_fetch_assoc($placeres)) {
            $placedatas[] = $placerow;
            
        }
        
    }
    mysqli_free_result($placeres);
}
else 
{
    echo "{$sql} 語法執行失敗，錯誤訊息：" . mysqli_error($_SESSION['db_conn']);
...
if(!empty($placedatas)){
	foreach($placedatas as $key => $value){
		$placeinfo=$value;
    ...
    <div class="row bg-info container-fluid" style="height:75px ;padding:0px;margin:0px;">
      ...
      <button type="button" class="btn btn-info col-sm-1 col-2" onclick="delete_cookie()">登出</button>
		...
		<div class="content">
            ...
            <label for="example-text-input" class="col-2 col-form-label">物品名稱</label>
            <div class="col-10">
              <input class="form-control" type="text" name="itemInfo" id="text_info" value="<?php echo $userinfo['itemInfo'];?>">
              <input type="hidden" class="form-control" type="text" name="itemInfo" id="text_id" value="<?php echo $userinfo['itemId'];?>">
            </div>
          </div>
          <!-- 數量 -->
          <div class="form-group row">
            <label for="example-text-input" class="col-2 col-form-label">數量</label>
            <div class="col-10">
              <input class="form-control" type="text" name="quan" id="text_quan" value="<?php echo $userinfo['quan'];?>" onkeyup="if(isNaN(value))execCommand('undo')" onafterpaste="if(isNaN(value))execCommand('undo')">
            </div>
          </div>
          <!-- 地點 -->
          <div class="form-group row">
            <label for="example-text-input" class="col-2 col-form-label">地點</label>
            <div class="col-10">
              <select class="form-control" id="place"  >
                <option value="nochoice" SELECTED  disabled>--選擇地點--</option>   
        			  	<?php foreach($placedatas as $row): ?>
          ...
          <div class="form-group row">
            ...
            <a href="http://35.221.196.173/api/file/image/<?php echo $userinfo['Image'];?>" target="_blank"><img src="http://35.221.196.173/api/file/image/<?php echo $userinfo['Image'];?>" style="height:100px" alt="ImgUrlNull"></a>
          <!-- 圖片上傳 -->  
          ...
          <div class="row">
            <div class="col-12">
              <button type="update all Data" class="btn btn-primary float-right" onclick=ItemAdd()>送出</button>
              <button style="margin-right: 10px;" type="button" class="btn btn float-right" onclick="history.back();">取消</button>
            </div>
          </div>
          </form>
			</div>
